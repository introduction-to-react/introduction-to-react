...
You can install dependencies using the `npm install <package name>` (or short `npm i <package name>`) command.
...
To install a package and add it as a "devDependency" to your package.json (instead of a normal "dependency), run `npm install --dev <package name>` (or short `npm i -D <package name>`).
